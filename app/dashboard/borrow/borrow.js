    ...
    onSubmit: async () => {
      ...
      await addDoc(collection(db, "loans"), {
        ...
        .then(() => {
          ...
          alert(
            `You have successfully borrowed ${values.amount} at the rate of ${rate}`
          );
        })
        .catch((e) => {
          setOpsProgress(false);
    ...
    <main className="min-h-screen flex justify-center py-4 md:py-6 lg:py-8 px-4 md:px-12 lg:px-16 bg-gray-100">
      <div className="w-full md:w-[380px] flex flex-col gap-4 border border-gray-200 rounded-md bg-gray-50 p-2 md:p-6">
        <blockquote className="border-b border-gray-200 pb-3">
          <span className="font-thin text-lg text-blue-800">
            Get Instant Loans
          </span>
        </blockquote>
        <form onSubmit={handleSubmit}>
            ...
            <ul className="grid grid-cols-3 gap-2">
              {durations.map((item) => (
                <li
                  ...
                  className={`h-16 flex justify-center items-center bg-blue-700 text-white text-md uppercase rounded-md`}
                >
                  {item.days}days
                </li>
              ))}
          ...
          <div className="flex items-center gap-1 mt-1">
            <button
              type="submit"
              className="p-2 rounded-md bg-blue-600 text-white text-xl uppercase"
            >
              {" "}
              Get Loan
            </button>
            <CircularProgress
              style={{ display: !opsProgress ? "none" : "flex" }}
            />
          </div>
        </form>
